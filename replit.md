...
- **Navbar Tools dropdown**: 640px wide, two-column layout. Left half (`w-1/2`) holds the menu items. Right half is a decorative pharmacist photo (`src/assets/tools-menu-bg.jpeg`) anchored to the right at full menu height with `overflow-hidden` clipping the left of the image, plus the same hero-style overlays as the home video (gradient `from-black/20 via-black/40 to-[#2c1b3d]/90` mix-blend-multiply, `bg-[#2c1b3d]/40` tint, and an inset shadow). While the Tools menu is open, a `fixed inset-0 z-40 bg-black/55 pointer-events-none` overlay dims the rest of the page (fades 0.18s); navbar (z-50) and the Radix dropdown (z-50) sit above it. The image is imported as a project-local asset because Vite's `server.fs.strict` blocks `@assets` paths outside the workspace root.
